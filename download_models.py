...
Examples:
    ...
    python download_models.py --models "SD-1.5"
...
IGNORE_PATTERNS = [
    ...
    "rust_model*",
    "tf_model*",
    "*.ckpt",  # Skip old checkpoint format, use safetensors
]
...
def main(
    ...
    if list_only:
        ...
        for key, cfg in MODEL_REGISTRY.items():
            ...
            print(
                f" {key:20s} {cfg['kind']:10s} {cfg['category']:15s} "
                f"~{cfg['vram_gb']:>4.1f} GB{gate}"
            )
            print(f"    {cfg['notes']}")
            print()
        return
